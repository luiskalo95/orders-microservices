...
export default class BrokerInfrastructure implements RepositoryBroker {
  ...
  public async sendError(message: any): Promise<void> {
    ...
    channel.publish(nameExchange, "delivery.order_cancelled.error", Buffer.from(messageAsString));
  }

  public async sendConfirmationFeedback(message: any): Promise<void> {
    const channel = BrokerBootstrap.getChannel();
    const messageAsString = JSON.stringify(message);
    const nameExchange = "ORDER_DELIVERED_EXCHANGE";
    await channel.assertExchange(nameExchange, "fanout", { durable: true });
    channel.publish(nameExchange, "", Buffer.from(messageAsString));
  }
  ...
  public async receive(): Promise<void> {
    const channel = BrokerBootstrap.getChannel();
    const created = this.receiveMessageAccepted(channel, this.consumerAccepted.bind(this), "ORDER_PREPARE_EVENT");
  ...
  public async receiveMessageAccepted(channel: Channel, cb: (message: any) => void, queueName: string) {
  ...
  public async consumerOrderConfirmed(message: any) {
    ...
    const content: Partial<DeliveryEntity> = JSON.parse(message.content.toString());
    content.status = "COMPLETED";
    await this.deliveryInfrastructure.update(content.transaction, content.status);
    this.confirmMessageBroker(message);
  }

  public async consumerFailedError(message: any) {
    console.log("Mensaje recibido",JSON.stringify(message.content.toString()));
    const content: Partial<DeliveryEntity> = JSON.parse(message.content.toString());
  ...
  public async consumerAccepted(message: any) {
    const content: Partial<DeliveryEntity> = JSON.parse(message.content.toString());
    ...
    await this.sendConfirmationFeedback(deliveryEntity);
    await this.sendOrRejectDelivery(deliveryEntity);
    this.confirmMessageBroker(message);
  }
  ...
  public sendOrRejectDelivery(deliveryEntity: DeliveryEntity): Promise<void>{
